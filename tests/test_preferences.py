...
def test_group_preferences_visible_to_members(client, verifier, sender):
    ...
    by_uid = {entry["uid"]: entry for entry in response.json()}
    assert by_uid["user-a"]["preferences"]["food_drink"]["mealBudget"] == "$$"
    assert by_uid["user-b"]["preferences"]["logistics"]["pace"] == "balanced"
...
def test_completion_status_matrix(client, verifier, sender):
    ...
    assert response.status_code == 200
    matrix = {entry["uid"]: entry["filled"] for entry in response.json()}
