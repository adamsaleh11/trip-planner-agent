...
Replace the hardcoded 11-friend agent graph with builder functions that construct, per request, five category specialist agents and a coordinator — instructions templated from trip context and participant preferences, output as a strict structured itinerary schema.
...
- [ ] Unit tests (mocked tools + stubbed LLM where needed) prove: instructions contain the actual participant preferences; empty category triggers the inference path; dietary restriction appears as a hard filter in the food agent's instruction.
