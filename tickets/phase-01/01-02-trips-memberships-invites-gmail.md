...
The full multi-tenant trip lifecycle over HTTP: create a trip (creator becomes admin), invite by email via the Gmail API with a copyable-link fallback, accept an invite, list members — with tenant isolation enforced at every endpoint.

## Responsibilities

- `POST /trips` (name, destination {text, lat, lng, placeId}, startDate, endDate, lodgingArea optional) → creates `trips/{tripId}` with `status: planning`, `adminUid`, plus `memberships/{uid}` subdoc `{role: admin}`, and appends to creator's `memberTripIds`.
- `GET /trips` (mine), `GET /trips/{id}` (members only), `PATCH /trips/{id}` (admin only: name/dates/lodging), `DELETE` deferred — out of scope.
- `POST /trips/{id}/invites` (admin only, body: email) → creates `invites/{token}` (token = URL-safe secrets token as doc ID; uniqueness for free) with email, tripId, invitedBy, status pending, createdAt. Sends email via Gmail API; response always includes `inviteUrl` so the UI can offer copy-link even if send fails (send failure is logged, not fatal).
...
- `POST /invites/{token}/accept` (authenticated) → idempotent: creates `memberships/{uid}` `{role: member}`, sets invite status accepted, appends to `memberTripIds`. Accepting an already-accepted invite by the same user → 200; by a different user → 410.
