...
> Source PRD: grill-me architecture session (2026-06-10) — multi-tenant trip journal with per-category preference agents, coordinator itinerary generation, email invites, anonymous collective trip memory (RAG), and a MapCN journal map.
...
- **Agents**: ADK agents constructed per-request with trip-parameterized instructions (no hardcoded people/rules). Coordinator fans out to category agents, merges into a structured itinerary: days → Morning/Afternoon/Evening blocks → timed stops with place_id, lat/lng, address, transport, "why it fits", and honest `suggested` provenance. `suggested=false` only when the item directly traces to explicit current-trip participant preferences; `suggested=true` for generic, inferred, padding, or memory-inspired recommendations.
- **Independent per-agent runs (2026-06-10)**: every category agent has its own visible UI div/panel and can be run standalone via `POST /trips/{id}/categories/{category}/generate`, writing candidates to `trips/{id}/categoryResults/{category}` (member-readable via listener). The coordinator/itinerary agent composes from fresh stored category results + all participants' preferences + admin manual plans, auto-running missing/stale categories. Staleness (preferences edited after a run) is advisory in the UI.
- **Manual plans as required context (2026-06-10 checkpoint)**: admins can add concrete plans manually using guided fields, not open-ended text: category, activity/name, time of day (`morning|afternoon|evening`), optional date, optional address/placeId/notes. These plans are mandatory coordinator context and must be included or explicitly scheduled in the generated itinerary unless impossible due to trip dates/location constraints.
...
- **External services**: Google Places Text Search + Routes API (existing tools, kept; destination search is already wired for trip creation and persists destination text/lat/lng/placeId), Vertex AI (Gemini via ADK, text-embedding-005, Vector Search), Gmail API (invite emails from the owner account; copyable invite link as fallback), MapCN (journal map: dark treatment, heatmap-style glow + waypoints).
...
3. **T2.3 — Category preference forms** *(skill: frontend-tdd)*
   The 5 category forms: chip/toggle groups for structured fields + free-text wishlist with example placeholders drawn from real phrasing ("sunset hikes", "best gelato", "bar to watch the game"); per-participant completion indicators on the trip page; saved preferences visibly rehydrate on return; empty categories clearly marked "AI will fill this".
...
- [ ] All 5 category forms save and visibly reload correctly; users can always see their saved preferences instead of wondering whether a save worked; completion states update per participant.
...
**User stories**: any member runs any category agent on its own and sees its recommendations; every category has its own generate button and result panel; admins can add manual plans that the AI must honor; any member clicks Generate Itinerary; watches agents work live; empty categories are AI-filled and labeled; the group gets a day-by-day itinerary with real venues built from the category agents' results, all participants' preferences, and admin manual plans.

### What to build

The core demo. Parameterized ADK agent graph + background execution + realtime progress + itinerary rendering. **Every agent is independently runnable with its own UI panel**: the 5 category agents each expose a run button and a visible results panel (stored in `trips/{id}/categoryResults/{category}`); the coordinator/itinerary agent composes from those stored results + all participants' preferences + admin manual plans, auto-running any missing or stale category first.
...
3. **T3.3 — Per-agent panels + generation UX + itinerary view** *(skill: frontend-tdd)*
   Each category card is an agent panel/div: Run button, live status (listener on `categoryResults/{category}`), candidate results list (venue, why-it-fits, suggested badge), stale-rerun hint when preferences changed after the run. Generate Itinerary button with pre-flight summary (which results are reused vs auto-run, which manual plans are mandatory, and which participants' preferences are included); live progress panel (5 agents + coordinator; reused categories render instantly-done); itinerary page: day sections, Morning/Afternoon/Evening blocks, timed stops with venue, address, transport, "why it fits", visible "AI-suggested" badges; regenerate action.
4. **T3.4 — "Right Now" spontaneity agent + API** *(skill: tdd)*
   `POST /whims` synchronous endpoint; single flash agent interprets the whim (time-of-day aware when empty), 1–2 Places queries, code-level random pick from qualified candidates, excludePlaceIds reroll support; trip-context flavor; whim docs persisted with metrics; collective-memory tip slot stubbed until Phase 4. Also patches the backend generation context so admin-authored manual plans are stored and automatically included in coordinator itinerary generation.
...
- [ ] Saved preferences are visible after reload/return for every participant/category, including admin-filled unclaimed participants.
- [ ] Any single category agent runs standalone from its own category div/panel and renders grounded candidates; editing that category's preferences afterwards surfaces a stale-rerun hint.
- [ ] Admin-created manual plans are visible before generation and automatically included in coordinator context.
- [ ] Generate Itinerary reuses fresh category results (`skipped_fresh` rendered as reused), auto-runs missing/stale ones, includes every participant's preferences plus manual plans, and produces a valid structured itinerary where every stop has a real Places place_id and inferred content is flagged `suggested`.
...
2. **T4.2 — Journal map page** *(skill: frontend-tdd)*
   MapCN dark map treatment; consume existing Google Places-derived destination lat/lng/placeId from trips; render trips as glowing waypoints over a heatmap-style layer weighted by visit density; fly-to on select; trip popover/sheet showing itinerary summary, members, and what each member wanted (trip-members only); journal list view alongside the map; loading/empty/missing-coordinate/provider-failure states polished.
